...
(function($) {
    ...
    // Guard: prevent runtime errors if jQuery tooltip plugin is not present
    // Some third-party snippets may call $(...).tooltip() (Bootstrap 4 style).
    // In Bootstrap 5, tooltip is a native class (no jQuery plugin). We define a
    // harmless no-op to avoid breaking the page when such calls exist.
    if (window.jQuery && window.jQuery.fn && typeof window.jQuery.fn.tooltip === 'undefined') {
    ...
    // JQuery counterUp (guarded if plugin not available)
    if ($.fn && $.fn.counterUp) {
    ...
    // Tooltip initialization (Bootstrap 5 compatible, no jQuery plugin usage)
    // We unify tooltip init here and avoid double initialization elsewhere.
    document.addEventListener('DOMContentLoaded', function () {
        if (window.bootstrap && typeof window.bootstrap.Tooltip === 'function') {
            document.querySelectorAll('[data-bs-toggle="tooltip"]').forEach(function (el) {
                // Prevent duplicate init
                if (!el.__tooltipInitialized) {
                    new window.bootstrap.Tooltip(el);
                    el.__tooltipInitialized = true;
                }
            });
        }
    });
    ...
    // Tooltip (removed duplicate initialization)
    // Tooltips are initialized on DOMContentLoaded above to avoid double init.
